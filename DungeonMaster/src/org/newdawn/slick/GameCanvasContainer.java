 ...
 * @author kevin
 */
public class GameCanvasContainer extends Canvas {
	private static final long serialVersionUID = 8464017451114419520L;
	
	/** Whether GL context sharing should be enabled */
	 ...
	 */
	public GameCanvasContainer(Game game) throws SlickException {
		this(game, false);
	}
	 ...
	 * @throws SlickException Indicates a failure during creation of the container
	 */
	public GameCanvasContainer(Game game, boolean shared) throws SlickException {
		super();
		...
		SwingUtilities.invokeLater(new Runnable() {
			public void run() {
				try {
					...
					try {
						Display.setParent(GameCanvasContainer.this);
	...
	private void scheduleUpdate() {
		if (!isVisible() || !container.running()) {
			return;
		}
	...
	private class Container extends AppGameContainer {
		...
		public Container(Game game, boolean shared) throws SlickException {
			super(game, GameCanvasContainer.this.getWidth(), GameCanvasContainer.this.getHeight(), false);

			width = GameCanvasContainer.this.getWidth();
			height = GameCanvasContainer.this.getHeight();
		...
		protected boolean running() {
			return super.running() && GameCanvasContainer.this.isDisplayable();
		}
		...
		public int getHeight() {
			return GameCanvasContainer.this.getHeight();
		}
		 ...
		 */
		public int getWidth() {
			return GameCanvasContainer.this.getWidth();
		}
		...
		public void checkDimensions() {
			if ((width != GameCanvasContainer.this.getWidth()) ||
			    (height != GameCanvasContainer.this.getHeight())) {
				
				try {
					setDisplayMode(GameCanvasContainer.this.getWidth(), 
								   GameCanvasContainer.this.getHeight(), false);
				} catch (SlickException e) {
					Log.error(e);
				}
			}
